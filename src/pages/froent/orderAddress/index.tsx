  ...
  return (
    <div className='content' style={{marginTop: 20}}>
      <div className={styles.addressWrap}>
        <Bread breads={['确认订单']}/>
        <div className="tit">选择收获地址
        <a href="/#/userCenter/address" style={{float: 'right', fontSize: 15}}>管理地址</a></div>
        <div className="addr-list">
         {
          address.map((i:any, idx) =>  <div className={`item ${curIdx === idx && 'cur'}`} onClick={() => {
            setCurIdx(idx)
          }}>
          <span>收货人:</span>{i.receiveUser}
          <br />
          <span>联系方式:</span>{i.contactNumber}
          <br />
          <span>收货地址:</span>{i.address}
        </div>)
         }
        </div>
        ...
        <table style={{width: '100%', fontSize: 16, color: '#333',marginTop: 30}} className={styles.tableWrap}>
        ...
        {state?.prods?.map((j:any) =><>
          <tr  style={{width: 800, textAlign: 'center', lineHeight: '20px'}}>
           ...
           </tr>
            {
               ...
               <td style={{fontSize: 16, color: '#FE1515'}}>¥{ (i.productAmount) * (i.price || i.nowPrice) }</td>
               <td style={{fontSize: 16, color: '#FE1515'}}>¥{ (i.productAmount) * (i.price || i.nowPrice)}</td>
              </tr>)
            }
        </>)
        ...
        <div className="money-detail">
          <div className="items">
            <div className="item">
              <span className='label'>商品件数：</span> <span className="num">{totalCount}件</span>
            </div>
            <div className="item">
              <span className='label'>商品总价：</span> <span className="num">¥{totalPrice}</span>
            </div>
            <div className="item">
              <span className='label'>活动优惠：</span> <span className="num">-¥{0}</span>
            </div>
          </div>
          <div className="items">
            <div className="item">
              <span className='label'>运费：</span> <span className="num">0</span>
            </div>
            <div className="item">
              <span className='label'>应付总额：</span> <span className="num" style={{fontSize: 20}}>¥{totalPrice}</span>
            </div>
          </div>
        </div>
            ...
            if(!address.length) {
             ...
             const res = await commonRequest('/mallOrderMaster/addOrder', {
              ...
              data:{
                ...
                "productVos": allProdus.map((i: any) => ({ isCart: 0, num: 1, productId: i.id, type: i.type })),

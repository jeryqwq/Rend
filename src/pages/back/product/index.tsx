      ...
      <div className={styles.wrap}>
        ...
        {type === 'part' && (
          <Button
            ...
            style={{ float: 'right' }}
            onClick={async () => {
              await commonRequest('/equipmentParts/innerNum', {
                method: 'get',
              });
              tableRef.current?.reloadAndRest();
            }}
        ...
        <ProTable
          ...
          columns={[
            ...
            },
            {
              title: '库存',
              dataIndex: 'num',
              hideInTable: type !== 'part',
            },
            {
              title: type === 'part' ? '零件类型' : '设备类型',
          ...
          request={async ({ pageSize, current, ...others }, sort, filter) => {
            ...
            if (others?.equipTypeText?.length) {
              conditions.push({
                ...
                operator: 'eq',
                value:
                  others?.equipTypeText?.[others?.equipTypeText.length - 1],
              });
          ...
          options={false}
          tableExtraRender={() => (
            <span style={{ color: '#858585', fontSize: 18, padding: '0 30px' }}>
              商品总数: {tableRef.current?.pageInfo?.total}
            </span>
          )}
        />
      </div>
